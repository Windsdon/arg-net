...
module.exports = function (hostname, client) {
	...
	r.download = function (endpoint, opt, writeStream) {
		opt = opt || {};
		opt.url = mp(endpoint);
		return new Promise((resolve, reject) => {
			var stream = _request.get(opt).on('response', function (response) {
				if (response.statusCode != 200) {
					reject(new Error('That link is invalid - Status Code: ' + response.statusCode));
				} else {
